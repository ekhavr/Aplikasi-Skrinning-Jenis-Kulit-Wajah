...
if ($_GET['id']) {
  ...
  $sqluser = mysql_query("SELECT * FROM tb_user where id_user=" . $_GET['id']);
  while ($ruser = mysql_fetch_array($sqluser)) {
    $arnama = unserialize($ruser['nama']);
    $arusia = unserialize($ruser['usia']);
    $arjeniskelamin = unserialize($ruser['jenis_kelamin']);
  } 
