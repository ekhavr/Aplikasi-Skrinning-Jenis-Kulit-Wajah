...
switch($_GET[act]){
  ...
  $tampil=mysql_query("SELECT * FROM tb_gejala ORDER BY kd_gejala");
	...
	if ($_POST[Go]){
			$numrows = mysql_num_rows(mysql_query("SELECT * FROM tb_gejala where nama_gejala like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysql_query("SELECT * FROM tb_gejala where nama_gejala like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
	...
	if ($counter % 2 == 0) $warna = "dark";
       ...
       echo "<tr class='".$warna."'>
			 ...
			 <td>$r[nama_gejala]</td>
			 <td align=center><a type='button' class='btn btn-success margin' href=gejala/editgejala/$r[kd_gejala]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysql_query("SELECT * FROM tb_gejala ORDER BY kd_gejala limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysql_fetch_array($hasil)){
	if ($counter % 2 == 0) $warna = "dark";
       ...
       echo "<tr class='".$warna."'>
			 ...
			 <td>$r[nama_gejala]</td>
			 <td align=center>
			 <a type='button' class='btn btn-success margin' href=gejala/editgejala/$r[kd_gejala]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	          <a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=gejala&act=hapus&id=$r[kd_gejala]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\"><i class='fa fa-trash-o' aria-hidden='true'></i> Hapus</a>
             </td></tr>";
      $no++;
	  $counter++;
  ...
  case "editgejala":
    $edit=mysql_query("SELECT * FROM tb_gejala WHERE kd_gejala='$_GET[id]'");
