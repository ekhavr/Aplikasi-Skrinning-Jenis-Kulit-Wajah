...
$aksi="modul/kel-admin/aksi_admin.php";
switch($_GET[act]){
	// Tampil admin
  default:
  $offset=$_GET['offset'];
  ...
  $tampil=mysql_query("SELECT * FROM tb_admin ORDER BY username");
	echo "<br><form method=POST action='?module=kel-admin' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
	...
	if ($_POST[Go]){
			$numrows = mysql_num_rows(mysql_query("SELECT * FROM tb_admin where username like '%$_POST[keyword]%'"));
			if ($numrows > 0){
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>"; 
	$hasil = mysql_query("SELECT * FROM tb_admin where username like '%$_POST[keyword]%'");
	$no = 1;
	$counter = 1;
	...
	echo" <table class='table table-bordered' style='overflow-x=auto' cellpadding='0' cellspacing='0'>
		  ...
		  <tbody>
		  "; 
	$hasil = mysql_query("SELECT * FROM tb_admin ORDER BY username limit $offset,$limit");
	$no = 1;
	$no = 1 + $offset;
	$counter = 1;
    while ($r=mysql_fetch_array($hasil)){
  ...
  case "editadmin":
    $edit=mysql_query("SELECT * FROM tb_admin WHERE username='$_GET[id]'");
